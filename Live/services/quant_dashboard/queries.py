...
QUANT_TABLES = [
    "symbols", "experiment_runs", "strategy_runs", "backtest_runs",
    "walk_forward_runs", "universe_runs", "feature_snapshots",
    "risk_snapshots", "model_candidates", "data_quality_events",
]
...
def _preferred_backend(value: str | None = None) -> str:
    ...
    if val in {"sqlite", "postgres"}:
        return val
    env_backend = os.environ.get("ALGOTRADER_DB_BACKEND", "").strip().lower()
    return "postgres" if env_backend in {"postgres", "postgresql"} else "sqlite"
...
def _connect(repo_root: Path, backend: str):
    ...
    return connect_database(load_database_config(repo_root=str(repo_root), backend=backend))
...
def _rows_to_dicts(cursor: Any) -> list[dict[str, Any]]:
    cols = [str(item[0]) for item in (getattr(cursor, "description", None) or [])]
    out = []
    for row in cursor.fetchall():
        if isinstance(row, dict):
            ...
            data = {cols[i]: value for i, value in enumerate(row)} if cols else {"value": row}
        out.append({str(k): (v if isinstance(v, (str, int, float, bool)) or v is None else str(v)) for k, v in data.items()})
    return out

def _count_table(conn: Any, table: str) -> tuple[int, str | None]:
    try:
        row = _execute(conn, f"SELECT COUNT(*) AS count FROM {table}").fetchone()
        if isinstance(row, dict):
...
def _select_rows(conn: Any, table: str, *, order_by: str | None, limit: int) -> tuple[list[dict[str, Any]], str | None]:
    ordered_sql = f"SELECT * FROM {table} ORDER BY {order_by} LIMIT {int(limit)}" if order_by else f"SELECT * FROM {table} LIMIT {int(limit)}"
    try:
        return _rows_to_dicts(_execute(conn, ordered_sql)), None
    except Exception as first_exc:
        _rollback(conn)
        try:
            return _rows_to_dicts(_execute(conn, f"SELECT * FROM {table} LIMIT {int(limit)}")), None
        except Exception as second_exc:
            _rollback(conn)
            return [], f"{table}: {type(second_exc).__name__}: {second_exc}; ordered_query_error={type(first_exc).__name__}: {first_exc}"

def load_quant_dashboard(*, repo_root: str | Path | None = None, backend: str | None = None, limit: int = 10) -> QuantDashboardPayload:
    root = find_repo_root(repo_root)
    selected_backend = _preferred_backend(backend)
    limit = max(1, min(int(limit or 10), 100))
    errors: list[str] = []
    counts = {table: 0 for table in QUANT_TABLES}
    sections = {key: [] for key in SECTION_SPECS}
    try:
        ...
        return QuantDashboardPayload(status, selected_backend, str(root), counts, sections, errors[:25], "Quant dashboard loaded." if status == "PASS" else "Loaded with warnings.")
    except Exception as exc:
        return QuantDashboardPayload("FAIL", selected_backend, str(root), counts, sections, [f"{type(exc).__name__}: {exc}", traceback.format_exc(limit=8)], "Quant dashboard could not connect.")
