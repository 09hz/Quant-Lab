...
def _is_eligible_parent_run(payload: dict) -> bool:
...
def find_latest_parent_run(live_root: Path, explicit_run_id: str = "") -> tuple[Path | None, dict]:
    runs_dir = live_root / "data" / "auto_lab_runs"
    if not runs_dir.exists():
        return None, {}

    candidates = [runs_dir / explicit_run_id] if explicit_run_id else sorted(
    ...
    for run_dir in candidates:
        ...
        try:
            ...
            continue
        if _is_eligible_parent_run(payload):
...
def run_baseline_on_csv(
    *,
    ...
    goal = ExperimentGoal(
        question="CSV baseline run for v21.4 mutation parent discovery.",
        symbols=[symbol],
        timeframe="1d",
        ...
        notes="CSV historical bars; baseline only.",
    )
    orchestrator = AutoLabOrchestrator(adapter=CoreStrategyBacktestAdapter(), live_root=live_root)
    run = orchestrator.run_experiment(goal=goal, candidates=seeds, bars_by_symbol={symbol: bars})
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Run Auto Lab mutation/retest loop on CSV historical bars.")
    ...
    parser.add_argument("--mutate-quantity", action="store_true", help="Disabled by default.")
    ...
    parent_run_dir, parent_payload = find_latest_parent_run(live_root, explicit_run_id=args.run_id)
    if not parent_payload:
        print("No eligible parent run found. Running CSV baseline seed test first.")
        baseline_run = run_baseline_on_csv(
            live_root=live_root,
            symbol=symbol,
            bars=bars,
    ...
    goal = ExperimentGoal(
        question=(
            "v21.4 CSV mutation retest loop: mutate research-pass parent strategies "
            "and retest on CSV historical bars."
        ),
        symbols=[symbol],
        timeframe="1d",
        starting_cash=12000.0,
        ...
        simulation_only=True,
        notes="CSV historical bars; quantity fixed unless explicitly enabled.",
    )
    ...
    settings = {
        ...
        "synthetic_vs_real_data": "csv_historical_bars",
        "simulation_only": True,
    }
    ...
    # Rewrite normal bundle after adding CSV labels.
    write_run_bundle(run, Path(run.artifacts["report_md"]).parent)
    ...
    # Rewrite one final time so experiment_run.json includes new artifacts.
    write_run_bundle(run, Path(run.artifacts["report_md"]).parent)
    ...
    print("AI Auto Lab CSV mutation retest complete.")
